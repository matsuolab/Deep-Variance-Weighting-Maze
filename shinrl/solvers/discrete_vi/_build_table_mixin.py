...
class BuildTableMixIn:
    def initialize(self, env: gym.Env, config: Optional[ViConfig] = None) -> None:
        ...
        self.data["QTarg"] = jnp.zeros((self.dS, self.dA))
        ...
        if self.config.approx == self.config.APPROX.nn:
            self.pred_q_all, self.pred_var_all = self._build_pred_all()
        self.update_tb_data()

    def update_tb_data(self):
        # Update Q values by q_net
        if self.config.approx == self.config.APPROX.nn:
            self.data["Q"] = self.pred_q_all(self.data["QNetParams"])
            self.data["QTarg"] = self.pred_q_all(self.data["QNetTargParams"])
            self.data["Var"] = self.pred_var_all(self.data["LogVarNetParams"])
            self.data["VarFrozen"] = self.pred_var_all(self.data["LogVarNetFrozenParams"])
    ...
    def _build_pred_all(self):
        pred_q_all = lambda params: self.q_net.apply(params, self.env.mdp.obs_mat)
        pred_var_all = lambda params: jnp.exp(self.var_net.apply(params, self.env.mdp.obs_mat))
        return jax.jit(pred_q_all), jax.jit(pred_var_all)
