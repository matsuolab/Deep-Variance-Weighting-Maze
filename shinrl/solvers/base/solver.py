...
class BaseSolver(ABC, History):
    ...
    def initialize(
        ...
        if self.config.verbose:
            mixins, methods = self.mixins, self.methods_str
    ...
    def set_env(self, env: gym.Env, reset: bool = True) -> None:
        ...
        if self.is_shin_env:
            if self.config.discount != env.config.discount:
                self.logger.warning(
                    f"env.config.discount != solver.config.discount \
                    ({env.config.discount} != {self.config.discount}). \
        ...
        if reset:
            ...
            obs_assert_msg = 'env must have attribute "obs". Do env.obs = obs before calling "set_env".'
            assert hasattr(env, "obs"), obs_assert_msg
    ...
    def run(self) -> None:
        ...
        if self.config.verbose:
            summary = self.recent_summary(num_steps)
            data = list(self.data.keys())
            msg = f"Epoch {self.n_epoch} has ended."
            self.logger.info(msg, epoch_summary=summary, data=data)
